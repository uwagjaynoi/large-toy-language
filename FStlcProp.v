...
Ltac iac H := inversion H; subst; clear H.
...
Proof.
  ...
  - iac Ht. discriminate.
  - iac Ht. right.
    specialize (IHt1 _ H2). specialize (IHt2 _ H4).
    destruct IHt1.
  + destruct IHt2.
  * eapply canonical_forms_fun in H. 2:{ apply H2. }
    destruct H as [x [u H]]; subst.
    exists <{[x:=t2]u}>. constructor. assumption.
  * destruct H0 as [t2' H0]. exists <{t1 t2'}>.
    constructor; assumption.
  + destruct H as [t1' H]. exists <{t1' t2}>.
    constructor; assumption.
  - iac Ht.
    specialize (IHt1 _ H3).
    specialize (IHt2 _ H5).
    specialize (IHt3 _ H6).
    destruct IHt1 as [vt1|[t1' Ht1]].
  + eapply canonical_forms_bool in vt1; try(assumption); right.
    destruct vt1 as [eq|eq]; subst t1.
...
Proof.
  ...
  + rewrite update_eq in H. injection H as H. subst.
    apply weakening_empty. assumption.
  + constructor. rewrite <- H. rewrite update_neq; eauto.
  - destruct (eqb_spec x x0); subst; constructor.
...
Proof.
  exists <{(\x:Bool->Bool, \y:Bool, x) (\z:Bool,y)}>.
  exists <{\y:Bool, \z:Bool, y}>.
  exists <{Bool -> Bool -> Bool}>. repeat split; repeat constructor.
  intro contra. iac contra. iac H4. iac H6.
  discriminate.
...
Proof.
  ...
  induction Hmulti.
  - destruct (progress _ _ Hhas_type); intuition.
  - assert (H2 := preservation _ _ _ Hhas_type H).
    intuition.
Qed.
(** [] *)
...
Proof with try(congruence).
  ...
  induction e; intros Gamma T T' HT HT'; iac HT; iac HT'...
  - specialize (IHe1 _ _ _ H2 H3)...
  - f_equal. specialize (IHe _ _ _ H4 H5)...
  - specialize (IHe2 _ _ _ H5 H8)...
Qed.
(** [] *)
...
Proof.
  intros t T H1 x H2.
  eapply free_in_context in H2. 2:{ apply H1. }
...
Proof.
  ...
  - constructor. rewrite <- H. symmetry.
    apply H0. constructor.
  - constructor. apply IHhas_type.
    intros. unfold update, t_update. destruct (eqb_spec x0 x1); try(reflexivity).
    apply H0. constructor; assumption.
  - apply T_App with T2.
  + apply IHhas_type1. intros. apply H1. eauto using appears_free_in.
  + apply IHhas_type2. intros. apply H1. eauto using appears_free_in.
Qed.
(** [] *)
...
(** **** Exercise: 2 stars, standard (stlc_variation2)
    ...
    false, give a counterexample.
      ...
      - Determinism of [step]
        (* F, (\x:Bool, x) tru to both tru and foo tru. *)
      - Progress
        (* T, cong. *)
      - Preservation
        (* F, (\x : A, x) --> true *)
...
(** **** Exercise: 2 stars, standard, optional (stlc_variation4)
    ...
    false, give a counterexample.
      ...
      - Determinism of [step]
        (* F, if true then false else false. *)
      - Progress
        (* T, triv. *)
      - Preservation
        (* F, if true then (\x:Bool,x) else (\x:Bool,x). *)
*)
(** [] *)
...
(** **** Exercise: 2 stars, standard, optional (stlc_variation7)
    ...
    false, give a counterexample.
      ...
      - Determinism of [step]
          (* T, cong. *)
      - Progress
          (* F, if(\x:Bool,x)then tru else tru. *)
      - Preservation
          (* T. *)
*)
(** [] *)
...
Inductive step : tm -> tm -> Prop :=
  ...
  | ST_Succ : forall t1 t2,
         t1 --> t2 ->
         <{succ t1}> --> <{succ t2}>
  | ST_SuccNum n : tm_succ (tm_const n) --> tm_const (S n)
  | ST_Pred : forall t1 t2,
         t1 --> t2 ->
         <{pred t1}> --> <{pred t2}>
  | ST_PredNum n: tm_pred (tm_const n) --> tm_const (pred n)
  ...
  | ST_Mult2 : forall t1 t2 t2',
         ...
         t2 --> t2' ->
         <{t1 * t2}> --> <{t1 * t2'}>
  | ST_MultNum n m x : n * m = x -> tm_mult (tm_const n) (tm_const m) --> tm_const x
  | ST_If : forall t1 t2 t3 t1',
         t1 --> t1' ->
         <{if0 t1 then t2 else t3}> --> <{if0 t1' then t2 else t3}>
  | ST_If0 : forall t2 t3,
         tm_if0 (tm_const 0) t2 t3 --> t2
  | ST_IfSucc n: forall t2 t3,
...
Proof. exists 6.
  econstructor.
  - econstructor.
    eapply ST_AppAbs. constructor.
  - simpl. econstructor.
  + eapply ST_AppAbs. constructor.
  + simpl. apply multi_R. constructor. reflexivity.
Qed.
...
Inductive has_type : context -> tm -> ty -> Prop :=
  ...
  | T_Succ : forall Gamma t, Gamma |-- t \in Nat -> Gamma |-- <{succ t}> \in Nat
  | T_Pred : forall Gamma t, Gamma |-- t \in Nat -> Gamma |-- <{pred t}> \in Nat
  | T_Mult : forall Gamma t1 t2, Gamma |-- t1 \in Nat -> Gamma |-- t2 \in Nat -> Gamma |-- <{t1 * t2}> \in Nat
...
Lemma weakening_empty : forall Gamma t T,
     ...
     Gamma |-- t \in T.
Proof.
  intros Gamma t T. apply weakening.
  discriminate.
Qed.
...
Lemma substitution_preserves_typing : forall Gamma x U t v T,
  ...
  Gamma |-- [x:=v]t \in T.
Proof.
  intros. remember (x0 |-> U; Gamma) as Gamma'.
  generalize dependent Gamma.
  induction H; subst; simpl; intros Gamma' HeqGamma; try(econstructor; eauto; fail).
  - destruct (eqb_spec x0 x1); subst.
  + rewrite update_eq in H. apply weakening_empty. congruence.
  + rewrite update_neq in H; try(assumption).
    constructor. assumption.
  - destruct (eqb_spec x0 x1); subst; constructor.
  + rewrite update_shadow in H. assumption.
  + apply IHhas_type. rewrite update_permute; auto.
Qed.
...
Proof with eauto.
  ...
  intros T IH1; iac IH1; try(econstructor; eauto; fail); eauto.
  iac H3. eapply substitution_preserves_typing; eauto.
Qed.
(** [] *)
...
Theorem progress : forall t T,
  ...
  value t \/ exists t', t --> t'.
Proof with eauto.
  intros. remember empty as Gamma. induction H; subst Gamma; try(discriminate).
  - left. constructor.
  - right. destruct (IHhas_type1 eq_refl) as [v1|[t1' H1]].
  + destruct (IHhas_type2 eq_refl) as [v2|[t2' H2]].
  * iac v1; try(iac H; fail). exists <{[x0:=t2]t}>...
  * exists <{t1 t2'}>...
  + exists <{t1' t2}>...
  - left. constructor.
  - destruct (IHhas_type eq_refl) as [v|[t' H2]].
  + iac v; try(iac H; fail). right. exists (tm_const (S n))...
  + right. exists <{succ t'}>...
  - destruct (IHhas_type eq_refl) as [v|[t' H2]].
  + iac v; try(iac H; fail). right. exists (tm_const (pred n))...
  + right. exists <{pred t'}>...
  - right. destruct (IHhas_type1 eq_refl) as [v1|[t1' H1]].
  + destruct (IHhas_type2 eq_refl) as [v2|[t2' H2]].
  * iac v1; try(iac H; fail).
    iac v2; try(iac H0; fail). exists (tm_const (n*n0))...
  * exists <{t1 * t2'}>...
  + exists <{t1' * t2}>...
  - right. destruct (IHhas_type1 eq_refl) as [v1|[t1' H2]].
  + iac v1; try(iac H; fail).
    destruct n.
  * exists <{t2}>...
  * exists <{t3}>...
