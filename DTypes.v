...
Proof.
  ...
  - intros [t' H]. inversion H. inversion H1.
  - intros [H|H]; inversion H. inversion H1.
Qed.
(** [] *)
...
Proof.
  intros t [H|H].
  - destruct H; intros [t' H]; inversion H.
  - induction t; try(inversion H; fail).
  + intros [t' H2]; inversion H2.
  + iac H. intros [t' H]. iac H. apply IHt; intuition. exists t1'. assumption.
  (* - induction H; intros [t' H2]; inversion H2.
    apply IHnvalue. exists t1'. assumption. *)
Qed.
...
Ltac bnval_step :=
  try match goal with
  H : tru --> _ |- _ => inversion H end;
  try match goal with
  H : fls --> _ |- _ => inversion H end;
  try match goal with
  H : zro --> _ |- _ => inversion H end;
  try match goal with
  H1 : scc ?x --> _, H2 : nvalue ?x |- _ => inversion H1 end;
  try match goal with
  H1 : ?x --> _, H2 : nvalue ?x |- _ => destruct (value_is_nf _ (or_intror H2) (ex_intro _ _ H1)) end.

Theorem step_deterministic:
  deterministic step.
Proof with eauto.
  (* intros x; induction x; intros y1 y2 H1 H2; bnval_step; iac H1; iac H2; f_equal; try(reflexivity); bnval_step; eauto. *)

  intros x y1 y2 H1; generalize dependent y2;
  induction H1; intros y2 H2; iac H2; f_equal; eauto; bnval_step.
Qed.
(** [] *)
...
Proof with auto.
  ...
  - destruct IHHT as [|[]].
    + left. right. apply (nat_canonical t1 HT) in H...
    + right. exists <{succ x}>...
  - right. destruct IHHT as [|[]].
    + apply (nat_canonical t1 HT) in H. induction H.
      * exists <{0}>...
      * iac HT. exists t...
    + exists <{pred x}>...
  - right. destruct IHHT as [|[]].
...
Theorem preservation' : forall t t' T,
  ...
  |-- t' \in T.
Proof with eauto.
  intros t t' T H1 H. generalize dependent T. induction H; intros T H0; iac H0...
...
Proof.
  right. intros contra.
  specialize (contra <{if true then 0 else true}> <{0}> Nat).
  assert (H : <{ if true then 0 else true }> -->
  <{ 0 }> /\ |-- <{ 0 }> \in Nat).
  { split; auto. }
  apply contra in H.
  inversion H. solve_by_invert.
Qed.
(** [] *)
...
(** **** Exercise: 2 stars, standard (variation1)
   ...
   counterexample.
      - Determinism of [step]
            (* T, cong. *)
      - Progress
            (* F, |-- succ false \in Bool. *)
      - Preservation
            (* T. for |-- t \in T and t --> t', if the first one is not constructed by T_SuccBool, then the same in induction applies. if it is, t=succ t0, then succ t0 --> t', the only choice is t'=succ t0' and t0 --> t0', and by induction. *)
*)
(* Do not modify the following line: *)
...
(** **** Exercise: 2 stars, standard, optional (variation3)
   ...
   this rule?  For each one that does, give a counter-example.
      - Determinism of [step]
            (* F, if true then pred 0 else 0 to both pred 0 and if true then 0 else 0. *)
      - Progress
            (* T, trivial. *)
      - Preservation
            (* T, by induction. *)
...
(** **** Exercise: 2 stars, standard, optional (variation4)
   ...
   this rule?  For each one that does, give a counter-example.
      ...
      - Progress
            (* T, trivial. *)
      - Preservation
            (* T, new rule does not contribute to normalizing to value. *)
*)
(** [] *)
...
(** **** Exercise: 2 stars, standard, optional (variation5)
   ...
   this rule?  For each one that does, give a counter-example.
      - Determinism of [step]
            (* T, cong. *)
      - Progress
            (* F, if 0 then 0 else 0 \in Nat. *)
      - Preservation
            (* T. Weird because typing isn't a function. 0 cannot step. *)
*)
(** [] *)
...
(** **** Exercise: 1 star, standard (remove_pred0)
  ...
  then pred 0 /in Nat but cannot progress.
*)
(* Do not modify the following line: *)
