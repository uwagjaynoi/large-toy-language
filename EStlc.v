...
Inductive substi (s : tm) (x : string) : tm -> tm -> Prop :=
  ...
  | s_var2 (y : string) (eq : x <> y) :
      substi s x (tm_var y) (tm_var y)
  | abs_var1 T t1 :
      substi s x <{\x:T, t1}> <{\x:T, t1}>
  | abs_var2 T y (eq : x <> y) t1 t1' : substi s x t1 t1' ->
      substi s x <{\y:T, t1}> <{\y:T, t1'}>
  | app_var t1 t2 t1' t2': substi s x t1 t1' -> substi s x t2 t2' ->
      substi s x <{t1 t2}> <{t1' t2'}>
  | tru : substi s x <{true}> <{true}>
  | fls : substi s x <{false}> <{false}>
...
Proof.
  split.
  Hint Constructors substi : core.
  - generalize dependent t'. induction t; simpl; eauto; intros; subst; try(constructor; eauto; fail);
    destruct (x0 =? s0)%string eqn:E; try(rewrite eqb_eq in E); try(rewrite eqb_neq in E); try(subst); eauto.
  - intros H. induction H; simpl; try(rewrite eqb_refl); try(rewrite <- eqb_neq in eq; rewrite eq); try(f_equal); try(reflexivity); try(assumption).
Qed.
(** [] *)
...
Proof.
  eapply multi_step.
  - eapply ST_App1. eapply ST_AppAbs. constructor.
  - simpl. eapply multi_R. eapply ST_AppAbs. constructor.
Qed.
...
Proof.
  ...
  - apply T_App with (T2 := <{Bool}>); constructor; reflexivity.
Qed.
(** [] *)
...
Proof.
  ...
  repeat constructor.
  apply T_App with (T2 := <{Bool}>); try(constructor); try(reflexivity).
  apply T_App with (T2 := <{Bool}>); constructor; reflexivity.
Qed.
(** [] *)
...
Example typing_nonexample_3 :
  ~ (exists S T,
        empty |--
          \x:S, x x \in T).
Proof.
  assert (forall T2 T1:ty, T2 <> <{T2->T1}>).
  { induction T2; try(discriminate). intros T1 H. injection H as eq1 eq2.
    apply IHT2_1 with (T1 := T2_2). assumption. }
  intros [S [T TP]].
  iac TP. iac H5. iac H3. iac H6. injection H2. injection H3 as eq. rewrite eq. apply H.
Qed.
(** [] *)
