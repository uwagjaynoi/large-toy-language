...
Notation " T1 'to' T2 <<- e1 ;; e2" := (match e1 with
                              | <{{T1 -> T2}}> => e2
                              | _ => None
                              end)
         (right associativity, at level 60).
Notation " T1 'sum' T2 <<- e1 ;; e2" := (match e1 with
                              | <{{T1 + T2}}> => e2
                              | _ => None
                              end)
         (right associativity, at level 60).
Notation " T1 'prod' T2 <<- e1 ;; e2" := (match e1 with
                              | <{{T1 * T2}}> => e2
                              | _ => None
                              end)
         (right associativity, at level 60).
Notation " T1 == T2 ;; e" := (if eqb_ty T1 T2 then e else None) (right associativity, at level 60).
Notation " 'isNat' T ;; e" := (match T with
                            | <{{Nat}}> => e
                            | _ => None
                            end)
        (right associativity, at level 60).

Example a (T : ty) (e : ty -> option ty) : option ty :=
  isNat T ;; e T.
...
Fixpoint type_check (Gamma : context) (t : tm) : option ty :=
  ...
  | <{nil T}> => return <{{List T}}>
  | <{xh :: xt}> =>
    T <- type_check Gamma xh ;;
    T' <- type_check Gamma xt ;;
    <{{List T}}> == T' ;;
    return <{{List T}}>
...
Ltac destrust_ty T T1 T2 E :=
  destruct T as [T1 T2| |T1 T2|T1| |T1 T2] eqn:E;
  try discriminate.
...
Ltac unwrap' T1 T2 E :=
  match goal with
  | H : _ <- ?e ;; _ = return _ |- _ =>
    idtac "find1"; destruct e as [T1|] eqn:E; [simpl in H | discriminate]
  | H : (_ to _ <<- ?e ;; _) = return _ |- _ =>
    idtac "find2"; destruct e
  end;
  idtac "true".
Tactic Notation "unwrap" :=
  let T1 := fresh "T" in
  let T2 := fresh "T" in
  let E1 := fresh "Heq" in
  unwrap' T1 T2 E1.
...
Proof with eauto.
  intros Gamma t. generalize dependent Gamma.
  induction t; intros Gamma T Htc; inversion Htc; clear Htc.
  - (* var *) auto.
  - (* app *) repeat unwrap.
...
Fixpoint valuef (t : tm) : bool :=
  ...
  | <{ case t of | inl _ => _ | inr _ => _ }> => false
  | <{ nil _ }> => true
  | <{ t1 :: t2 }> => valuef t1 && valuef t2
  | <{ case t0 of | nil => _ | x21 :: x22 => _ }> => false
  | <{ unit }> => true
  | <{ (t1,t2) }> => valuef t1 && valuef t2
  | <{ t.fst }> | <{ t.snd }> => false
  | <{ let _ = _ in _ }> => false
...
Fixpoint stepf (t : tm) : option tm :=
  ...
  | <{ case t of | inl x1 => t1 | inr x2 => t2 }> =>
    ...
    | Some t', _ => Some <{ case t' of | inl x1 => t1 | inr x2 => t2 }>
    | None, <{inl _ t1'}> => assert (valuef t1') (Some <{[x1:=t1']t1}>)
    | None, <{inr _ t2'}> => assert (valuef t2') (Some <{[x2:=t2']t2}>)
    | None, _ => None
    end
  (* lists (the [tm_lcase] is given for free) *)
  | <{nil _}> => None
  ...
  | <{t.fst}> =>
    match stepf t, t with
    | Some t', _ => Some <{t'.fst}>
    | None, <{(t1,t2)}> => assert (valuef t1) (assert (valuef t2) (Some t1))
    | None, _ => None
    end
  | <{t.snd}> =>
    match stepf t, t with
    | Some t', _ => Some <{t'.snd}>
    | None, <{(t1,t2)}> => assert (valuef t1) (assert (valuef t2) (Some t2))
    | None, _ => None
    end
  | <{let x = t1 in t2}> =>
    match stepf t1 with
...
Proof.
  induction t; intros Hv; simpl in Hv;
  try(discriminate);
  try(econstructor; fail);
  try (rewrite andb_true_iff in Hv);
...
Proof.
  intros t Hv. induction Hv;
  try(reflexivity); simpl;
  try(rewrite IHHv); eauto;
  try(rewrite IHHv1); eauto;
  try(rewrite IHHv2); eauto.
Qed.
(** [] *)
...
Ltac rw_stepf :=
  ...
  H : value ?t |- _ => rewrite (value_stepf_nf t H)
  end;
  repeat match goal with
  H : value ?t |- _ => rewrite (complete_valuef t H)
  end; try(reflexivity);
  repeat match goal with
  H : stepf ?t = _ |- _ => rewrite H
  end; try(reflexivity).
