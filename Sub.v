...
(** **** Exercise: 2 stars, standard, especially useful (arrow_sub_wrong)
    ...
    Suppose A is a fixed Person, but not Student. let g := \f, f A, let f := \s, s.gpa.
    Then (g f) := (\f, f A) (\s, s.gpa)
    --> (\s, s.gpa) A
    --> A.gpa
...
(** **** Exercise: 2 stars, standard (proper_subtypes)
    ...
    false. Top -> Bool has no non-trivial subtype.
*)
...
Definition Employee : ty := <{String * (Top * (Integer * Top))}>.
...
Example sub_student_person :
  Student <: Person.
Proof.
  unfold Student, Person. eauto.
Qed.

Example sub_employee_person :
  Employee <: Person.
Proof.
  unfold Employee, Person. eauto.
Qed.
(** [] *)
...
Proof with eauto.
  eauto using sub_student_person.
Qed.
(** [] *)
...
Proof with auto.
  intros U Hs.
  remember <{Bool}> as V.
  induction Hs; intuition; subst; auto; discriminate.
Qed.
(** [] *)
...
Proof with eauto.
  ...
  induction Hs; intuition; subst; try(discriminate); eauto.
  - destruct (IHHs2 V1 V2 eq_refl) as [U1 [U2 [H1 H2]]].
    subst.
    destruct (IHHs1 _ _ eq_refl) as [U3 [U4 [H3 H4]]].
    subst.
    exists U3, U4. intuition; eauto.
  - injection HeqV as eq1 eq2. subst.
    eauto.
Qed.
(** [] *)
...
Proof with auto.
  intros U Hs.
  remember <{Unit}> as V.
  induction Hs; intuition; subst; eauto; try(discriminate).
Qed.
(** [] *)
...
Proof.
  ...
  induction Hs; intuition; subst; eauto; try(discriminate).
  - destruct (IHHs2 _ _ eq_refl) as [U1 [U2 [eq1 [H1 H2]]]]; subst.
    destruct (IHHs1 _ _ eq_refl) as [U3 [U4 [eq2 [H3 H4]]]]; subst.
    exists U3, U4; eauto.
  - injection HeqV as eq1 eq2; subst.
    exists S1, S2; eauto.
Qed.
...
Proof with eauto.
  ...
  induction H; intros T3 T4 eq; subst; try(injection eq as eq1 eq2; subst); try(solve_by_invert).
  - eauto.
  - destruct (sub_inversion_arrow _ _ _ H1) as [U1 [U2 [H2 [H3 H4]]]]. subst.
    eapply IHhas_type...
Qed.
(** [] *)
...
Lemma canonical_forms_of_Pair : forall Gamma s T1 T2,
  Gamma |-- s \in (T1 * T2) ->
  value s ->
  exists t1 t2, value t1 /\ value t2 /\ s = <{(t1,t2)}>.
Proof with eauto.
  intros Gamma s T1 T2 TT.
  remember <{T1 * T2}> as T.
  generalize dependent T2.
  generalize dependent T1.
  induction TT; intros T3 T4 eq vs; subst; try solve_by_invert...
  - (* T_Pair *)
    inversion vs. subst. eauto.
  - (* T_Sub *)
    destruct (sub_inversion_pair _ _ _ H) as [U1 [U2 [eq [H1 H2]]]]; subst.
    eapply IHTT...
Qed.
...
Proof with eauto.
  ...
  - (* T_Pair *)
    destruct IHHt1 as [vt1|[t1' s1]]...
    destruct IHHt2 as [vt2|[t2' s2]]...
  - (* T_First *)
    destruct IHHt as [vt|[t' s]]...
    destruct (canonical_forms_of_Pair _ _ _ _ Ht vt) as [t1 [t2 [vt1 [vt2 eq]]]]; subst.
    eauto.
  - (* T_Second *)
    destruct IHHt as [vt|[t' s]]...
    destruct (canonical_forms_of_Pair _ _ _ _ Ht vt) as [t1 [t2 [vt1 [vt2 eq]]]]; subst.
...
Lemma typing_inversion_var : forall Gamma (x:string) T,
  ...
  exists S,
    Gamma x = Some S /\ S <: T.
Proof with eauto.
  intros. remember (tm_var x) as x'. induction H; subst; try(discriminate).
  - exists T1. split...
    injection Heqx' as eq. subst...
  - destruct (IHhas_type eq_refl) as [S [H1 H2]]...
Qed.
(** [] *)
...
Lemma typing_inversion_app : forall Gamma t1 t2 T2,
  ...
  exists T1,
    ...
    Gamma |-- t2 \in T1.
Proof with eauto.
  intros. remember <{t1 t2}> as tapp.
  generalize dependent t2.
  generalize dependent t1.
  induction H; intros t5 t6 Heq; try(discriminate).
  - injection Heq as eq1 eq2. subst...
  - subst. destruct (IHhas_type _ _ eq_refl) as [T3 [H1 H2]].
    exists T3...
Qed.
(** [] *)
...
Proof with eauto.
  ...
  generalize dependent t2.
  generalize dependent t1.
  induction H; intros t5 t6 eq; try(discriminate); subst.
  - injection eq as eq1 eq2; subst...
  - destruct (IHhas_type _ _ eq_refl) as [T3 [T4 [H1 [H2 H3]]]].
    exists T3, T4...
Qed.

Theorem typing_inversion_fst : forall Gamma t T,
  Gamma |-- t.fst \in T ->
  exists T2,
    Gamma |-- t \in <{T * T2}>.
Proof.
  intros. remember <{t.fst}> as tfst.
  induction H; try(discriminate).
  - injection Heqtfst as eq; subst. eauto.
  - destruct (IHhas_type Heqtfst) as [T3 H1]. eauto.
Qed.

Theorem typing_inversion_snd : forall Gamma t T,
  Gamma |-- t.snd \in T ->
  exists T1,
    Gamma |-- t \in <{T1 * T}>.
Proof.
  intros. remember <{t.snd}> as tsnd.
  induction H; try(discriminate).
  - injection Heqtsnd as eq; subst. eauto.
  - destruct (IHhas_type Heqtsnd) as [T3 H1]. eauto.
Qed.
...
Proof with eauto.
  ...
  - (destruct (eqb_spec x x0)); subst; econstructor...
...
Proof with eauto.
  ...
  - (* T_Fst *)
    inversion HE; subst...
    destruct (typing_inversion_pair _ _ _ _ HT) as [T3 [T4 [H2 [H3 H4]]]].
    destruct (sub_inversion_pair _ _ _ H4) as [U1 [U2 [eq [H5 H6]]]].
    injection eq as eq1 eq2; subst...
  - (* T_Snd *)
    inversion HE; subst...
    destruct (typing_inversion_pair _ _ _ _ HT) as [T3 [T4 [H2 [H3 H4]]]].
...
(** **** Exercise: 2 stars, standard (variations)
    ...
    - Suppose we add the following typing rule:
                    ...
                    -----------------------------------    (T_Funny1)
                           Gamma |-- t \in T1->T2
      Both true.
      has_type'<->has_type, progress'<->progress.
    ...
    - Suppose we add the following reduction rule:
                             ...
                             unit --> (\x:Top. x)
      progress true.
      has_type'<->has_type. so progress' only has weaker consequence.
      unit in Unit, but (\x:Top, x) not.
    ...
    - Suppose we add the following subtyping rule:
                               ...
                               Unit <: Top->Top
      preservation true.
      <{unit unit}> has type top, but does not progress;
    ...
    - Suppose we add the following subtyping rule:
                               ...
                               Top->Top <: Unit
      progress true.
      <{(\x:Unit,x) (\t:Top,t)}> has type unit, but <{(\t:Top,t)}> does not.
    ...
    - Suppose we add the same reduction rule _and_ a new typing rule:
      ...
      progress true.
      <{unit true}> in Top, but <{true unit}> not.
    ...
    - Suppose we _change_ the arrow subtyping rule to:
                          ...
                          S1->S2 <: T1->T2
      progress true.
      <{(\x:Bool,if x then x else x)}> in Bool->Bool in Top->Bool
      so <{(\x:Bool,if x then x else x) unit}> in Bool. but <{if unit then unit else unit}> not.
*)
...
Lemma not_Top_sub_Bool : ~ (<{Top}> <: <{Bool}>).
Proof.
  intros H1.
  assert (H2 := sub_inversion_Bool _ H1). discriminate.
Qed.

Ltac inj1 H :=
  let eq1 := fresh"eq" in
  injection H as eq1; subst.
Ltac inj2 H :=
  let eq1 := fresh"eq" in
  let eq2 := fresh"eq" in
  injection H as eq1 eq2; subst.

Ltac inj_cons_eq := repeat
  (try match goal with
    H : <{_ -> _}> = <{_ -> _}> |- _
    => inj2 H end;
  try match goal with
...
Ltac sub_inv := repeat
  (try match goal with
    H : _ <: <{_ -> _}> |- _
    => destruct5 (sub_inversion_arrow _ _ _ H); clear H
  end;
  try match goal with
    H : _ <: <{_ * _}> |- _
    => destruct5 (sub_inversion_pair _ _ _ H); clear H
  end;
  idtac).

Ltac workNotSub := repeat
  match goal with
    H : ?T1 <: ?T2 -> _ |- _
    => let H1 := fresh"H" in assert (H1 : T1 <: T2); [solve[eauto] | specialize (H H1); clear H1]
  end;
  repeat (sub_inv; try discriminate; inj_cons_eq; eauto using not_Top_sub_Bool).
...
Proof.
  right. intros H.
  specialize (H Ty_Bool Ty_Top Ty_Top Ty_Top).
  workNotSub.
Qed.
(** [] *)
...
Proof.
  right. intros H.
  specialize (H Ty_Top Ty_Top Ty_Bool Ty_Top).
  workNotSub.
Qed.
(** [] *)
...
Lemma notSupSub : forall S T, S <: <{T -> S}> -> False.
Proof. induction S; intros T H; workNotSub. Qed.

Theorem formal_subtype_instances_tf_2d:
  TF (exists S,
         S <: <{S->S}>).
Proof. right. intros [S HS]. eauto using notSupSub. Qed.
(** [] *)
...
Theorem formal_subtype_instances_tf_2e:
  TF (exists S,
         <{S->S}> <: S).
Proof. left. exists Ty_Top. eauto. Qed.
(** [] *)
...
Proof. left. exists Ty_Top, Ty_Top. eauto. Qed.
(** [] *)
...
Fixpoint inf_dec (n : nat) : ty :=
  match n with
  | 0 => Ty_Top
  | S n' => Ty_Arrow Ty_Top (inf_dec n')
  end.
Hint Resolve inf_dec : core.

Lemma inf_dec_inj : forall i j, inf_dec i = inf_dec j -> i = j.
Proof.
  induction i;
  intros; destruct j;
  try(discriminate);
  try(reflexivity).
  injection H as H. eauto.
Qed.

Theorem inf_dec_is_dec : forall i, inf_dec (S i) <: inf_dec i.
Proof.
  induction i; simpl in *; eauto.
Qed.
...
Proof.
  left. exists inf_dec.
  split.
  - eauto using inf_dec_inj.
  - apply inf_dec_is_dec.
Qed.
(** [] *)
...
Theorem formal_subtype_concepts_tfh:
  TF (exists f : nat -> ty,
         ...
         (forall i, f i <: f (S i))).
Proof.
  left. exists (fun n => Ty_Arrow (inf_dec n) Ty_Top).
  split.
  - intros i j H1 H2. injection H2 as H2.
    eauto using inf_dec_inj.
  - intros i. eauto using inf_dec_is_dec.
Qed.
(** [] *)
...
Proof.
  ...
  specialize (H (Ty_Arrow Ty_Top Ty_Bool) H1); clear H1.
  destruct H as [S [H1 H2]].
  sub_inv. eapply H2. subst. f_equal.
  - eauto using sub_inversion_Top.
  - eauto using sub_inversion_Bool.
Qed.
(** [] *)
...
Lemma sup_inversion_Bool : forall U,
     <{Bool}> <: U ->
     U = <{Bool}> \/ U = <{Top}>.
Proof with auto.
  intros U Hs.
  remember <{Bool}> as V.
  induction Hs; intuition; subst; auto; try(discriminate).
  assert (H := sub_inversion_Top _ Hs2). auto.
Qed.

Lemma sup_inversion_Unit : forall U,
     <{Unit}> <: U ->
     U = <{Unit}> \/ U = <{Top}>.
Proof with auto.
  intros U Hs.
...
Lemma sup_inversion_Base : forall U s,
     <{Base s}> <: U ->
     U = <{Base s}> \/ U = <{Top}>.
Proof with auto.
  intros U s Hs.
...
Lemma sup_inversion_arrow : forall U V1 V2,
     <{V1->V2}> <: U -> U = <{Top}> \/
     exists U1 U2,
     U = <{U1->U2}> /\ U1 <: V1 /\ V2 <: U2.
Proof with eauto.
  ...
  generalize dependent V2. generalize dependent V1.
  induction Hs; intuition; subst; try(discriminate); eauto.
  - right. exists V1, V2. eauto.
  - destruct (IHHs1 _ _ eq_refl).
    + subst. assert (H := sub_inversion_Top _ Hs2)...
    + destruct5 H. subst.
      destruct (IHHs2 _ _ eq_refl)...
      destruct5 H. subst.
      right. exists U, U2. eauto.
  - inj_cons_eq. right. exists T1, T2. eauto.
Qed.

Lemma sup_inversion_pair : forall U V1 V2,
     <{V1 * V2}> <: U -> U = <{Top}> \/
     exists U1 U2,
     U = <{U1 * U2}> /\ V1 <: U1 /\ V2 <: U2.
Proof with eauto.
...
Theorem sub_antisym (S T : ty) : S <: T -> T <: S -> S = T.
Proof.
  ...
  - intros.
    eauto using sub_inversion_Bool.
  - intros.
    eauto using sub_inversion_Base.
  - intros. sub_inv; subst.
    sub_inv; inj_cons_eq.
    f_equal; eauto.
  - intros.
    eauto using sub_inversion_Unit.
  - intros. sub_inv; subst.
    sub_inv; inj_cons_eq.
    f_equal; eauto.
Qed.
...
Ltac destruct2 H :=
  ...
  let H1 := fresh"H" in
  destruct H as [U H1].
(* Check typing_inversion_pair. *)
Ltac ty_inv := repeat
  (try match goal with
    H : has_type _ (tm_abs _ _ _) _ |- _
    => destruct3 (typing_inversion_abs _ _ _ _ _ H); clear H
  end;
  try match goal with
    H : has_type _ (tm_var _) _ |- _
    => destruct3 (typing_inversion_var _ _ _ H); clear H
  end;
  try match goal with
    H : has_type _ (tm_app _ _) _ |- _
    => destruct3 (typing_inversion_app _ _ _ _ H); clear H
  end;
  try match goal with
    H : has_type _ tm_unit _ |- _
    => let H1 := fresh"H" in assert (H1 := typing_inversion_unit _ _ H); clear H
  end;
  try match goal with
    H : has_type _ (tm_pair _ _) _ |- _
    => destruct5 (typing_inversion_pair _ _ _ _ H); clear H
  end;
  try match goal with
    H : has_type _ (tm_fst _) _ |- _
    => destruct2 (typing_inversion_fst _ _ _ H); clear H
  end;
  try match goal with
    H : has_type _ (tm_snd _) _ |- _
    => destruct2 (typing_inversion_snd _ _ _ H); clear H
  end;
  idtac).
...
Proof.
  assert (H : (forall T,
  empty |-- <{(\p:T*Top, p.fst) ((\z:A, z), unit)}> \in <{A->A}> <-> T = <{A -> A}>)).
  {
    split.
    {
      intros contra.
      ty_inv;
      repeat rewrite update_eq in *;
      repeat (try sub_inv; try inj_cons_eq; subst).
      assert (A <: U4). {eauto. }
      assert (U4 <: A). {eauto. }
      assert (U3 <: A). {eauto. }
      assert (A <: U3). {eauto. }
      eauto using sub_antisym.
    }
    {
      intros. subst.
      repeat (econstructor; eauto).
...
Proof.
  assert (H : (forall T,
  empty |-- <{(\p:(A->A)*(B->B), p) ((\z:A, z), (\z:B, z))}> \in T <-> <{ (A -> A) * (B -> B) }> <: T)).
  {
    split.
    {
      intros contra.
  ...
  right. left. split.
  - exists <{(A -> A) * (B -> B)}>. intuition.
    + apply H. auto.
    + apply H. auto.
  - intros [Tl H1].
    assert (H2 : forall T, T <: Tl <-> <{(A -> A) * (B -> B)}> <: T). {intros T. rewrite <- H. eauto. }
    clear H H1.
    assert (H1 := proj2 (H2 Ty_Top) (S_Top _)).
    assert (H3 := sub_inversion_Top _ H1). subst.
    specialize (H2 A).
    destruct H2.
    specialize (H (S_Top _)).
    assert (eq := sub_inversion_Base _ _ H).
    discriminate.
Qed.
(** [] *)
...
Theorem formal_small_large_3:
  ...
  (fun T =>
   (a |-> A) |-- <{(\p:A*T, (p.snd) (p.fst)) (a, \z:A, z)}> \in A).
Proof.
  assert (H : forall T, (a |-> A) |-- <{(\p:A*T, (p.snd) (p.fst)) (a, \z:A, z)}> \in A <-> T = <{A -> A}>).
  {
    split.
    {
      intros contra.
      ...
      repeat (try sub_inv; try inj_cons_eq; subst).
      eauto.
      generalize dependent U3.
      generalize dependent U.
      introclear 5.
      assert (U17 <: A). {eauto. }
      generalize dependent U5.
      generalize dependent U10.
      introclear 5.
      assert (A <: U18). {eauto. }
      generalize dependent U9.
      generalize dependent U11.
      generalize dependent U1.
      introclear 7.
      generalize dependent U7.
      generalize dependent U8.
      introclear 4.
      assert (U18 <: A). {eauto. }
      generalize dependent U12.
      introclear 3.
      assert (A <: U17). {eauto. }
      generalize dependent U6.
      introclear 3.
      f_equal; eauto using sub_antisym.
    }
    {
      intros eq; subst.
      repeat (econstructor; eauto).
...
Theorem formal_small_large_4:
  ...
  (fun T =>
   exists S,
     empty |-- <{\p:A*T, (p.snd) (p.fst)}> \in S).
Proof.
  assert (H : forall T, (exists S,
  empty |-- <{\p:A*T, (p.snd) (p.fst)}> \in S) <-> T <: <{A -> Top}>).
  {
    split.
    {
      intros [S contra].
  ...
  split.
  - intros [Ts H1].
    specialize (H Ty_Top). specialize (H1 Ty_Top).
    rewrite H in H1.
    destruct H1. specialize (H0 (S_Top _)).
    assert (H3 := sub_inversion_Top _ H0).
    discriminate.
  - exists <{A -> Top}>. intuition.
    + apply H. auto.
    + apply H. auto.
Qed.
(** [] *)
...
(* Ltac try_trans := repeat
  match goal with
    U : ty |- ?H
    => generalize dependent U;
    (match goal with
    |- forall U, U <: ?U1 -> H => idtac U; introclear 2 end ||
    match goal with
    |- forall U, ?U1 <: U -> H => idtac U; introclear 2 end ||
    match goal with
    |- forall U, ?U1 <: U -> U <: ?U2 -> H =>
      let H1 := fresh"H" in
      let H2 := fresh"H" in
      let H3 := fresh"H" in
      idtac U; intros U H1 H2; assert (H3 : U1 <: U2); [eauto; fail | clear dependent U] end ||
    match goal with
    |- forall U, U <: ?U2 -> ?U1 <: U -> H =>
      let H1 := fresh"H" in
      let H2 := fresh"H" in
      let H3 := fresh"H" in
      idtac U; intros U H1 H2; assert (H3 : U1 <: U2); [eauto; fail | clear dependent U] end ||
...
(* Theorem ty_nonempty : forall T, exists t, empty |-- t \in T.
Proof.
  induction T; eauto.
  -  *)
...
Theorem formal_smallest_1:
  ...
  (fun T =>
   exists S t,
     empty |-- <{ (\x:T, x x) t }> \in S).
Proof.
  assert (H : forall T, (exists S t,
  empty |-- <{ (\x:T, x x) t }> \in S) <-> (exists T1 T2, T = <{T1 -> T2}> /\ <{T1 -> T2}> <: T1) /\ exists t, empty |-- t \in T).
  {
    split.
    {
      intros [S [t contra]].
      ...
      (* clear H0. *)
      (* assert (U5 <: S). {eauto. }
      generalize dependent U7. introclear 3.
      generalize dependent U2. introclear 2.
      generalize dependent U0. introclear 2. *)
      (* generalize dependent U0. introclear 2. *)
    }
    {
      (* split.  *)
      intros [[T1 [T2 [H1 H2]]] [t HT]]. exists T2, t.
      sub_inv; subst.
      repeat (econstructor; eauto).
    }
  }
  right. intros [TS H3].
  assert (H4 : TS <: <{Top -> Bool}>).
  {apply H3. apply H. split.
  - repeat eexists; eauto.
  - exists <{\"x":Top, true}>. auto. }
  assert (H5 : TS <: <{Top -> Unit}>).
  {apply H3. apply H. split.
  - repeat eexists; eauto.
  - exists <{\"x":Top, unit}>. auto. }
  apply sub_inversion_arrow in H4.
  apply sub_inversion_arrow in H5.
  destruct H4 as [U1 [U2 [Heq1 [_ H4]]]].
  destruct H5 as [U3 [U4 [Heq2 [_ H5]]]]. subst TS.
  apply sub_inversion_Bool in H4.
  apply sub_inversion_Unit in H5. subst.
  discriminate.
Qed.
(** [] *)
...
Theorem formal_smallest_2:
  ...
  (fun T =>
   empty |-- <{(\x:Top, x) ((\z:A, z), (\z:B, z))}> \in T).
Proof.
  assert (H : forall T, (empty |-- <{(\x:Top, x) ((\z:A, z), (\z:B, z))}> \in T) <-> (T = Ty_Top)).
  {
    split.
    {
      intros contra.
  ...
  left. exists Ty_Top. intros.
  rewrite H.
  split.
  - eauto using sub_inversion_Top.
  - intros eq. subst. eauto.
Qed.
(** [] *)
...
(*
  ...
  do progress & preservation hold?

  Progress:
    We claim that every value constructor has corresponding type constructor:
      abs -> Arrow
      nat -> Nat
      unit -> Unit
      loc -> Ref
    ...
    Induction on term.
    App case: typecheck, so left side is an Arrow, so value(then abs)/progress
              right value/progress
              subst correct
    Succ/Pred/Mult/If0 case: typecheck, so inner/condition is Nat, so value(then n)/progress
    ref case: ez
    deref case: inner is Ref _, so v(loc _)/progress
    assign case: l1 \in Ref T, l2 \in T
      l1 value(then loc _)/progress

  Assume A <: B are two types, b in B but not in A.
  f(x) is a pattern that can only progress and typecheck if x is in A, and at this time its type is C

  let A = {x:Int,y:Int}, B = {x:Int}, a = {x:=0, y:=0}, b = {x:=0}

  Preservation:
    Contra- case
    let exp = (!(loc 0)).y / [b]
        ST = [B]
    empty ; ST |-- (loc 0) in Ref B
    empty ; ST |-- (loc 0) in Ref A
    empty ; ST |-- !(loc 0) in A
    empty ; ST |-- (!(loc 0)).y in Int
    but (!(loc 0)).y / [b] --> b.y / [b]
    b.y does not type.

    Co- case
    let exp = (loc 0) := b ; (!(loc 0)).y / [a]
        ST = [A]
    empty ; ST |-- (loc 0) in Ref A (1)
    empty ; ST |-- (loc 0) in Ref B (2<-1)
    empty ; ST |-- (loc 0):=b in Unit (3<-2)
    empty ; ST |-- !(loc 0) in A (4<-1)
    empty ; ST |-- (!(loc 0)).y in Int (5<-4)
    empty ; ST |-- (loc 0) := b ; (!(loc 0)).y in Int (6<-3,5)
    but
    (\_:Unit, (!(loc 0)).y) ((loc 0) := b) / [a] --> (\_:Unit, (!(loc 0)).y) unit / [b]
    if it types D
    (!(loc 0)).y in D
    so D=Int and !(loc 0) in (a record with y), so (loc 0) in Ref (a record with y), but there's no y in [b].
*)
